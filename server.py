...
def free_trial_days() -> int:
    """Return the configured one-time free trial length."""
    return max(1, env_int("FREE_TRIAL_DAYS", 7))
...
class User(UserMixin, db.Model):
    ...
    def has_active_trial(self) -> bool:
        if self.is_admin or self.has_active_subscription:
            return False
        return bool(self.trial_end and self.trial_end > utcnow())

    @property
    def has_library_access(self) -> bool:
        return bool(self.is_admin or self.has_active_subscription or self.has_active_trial)

    @property
    def trial_days_left(self) -> int:
        if not self.has_active_trial or not self.trial_end:
            return 0
        seconds = (self.trial_end - utcnow()).total_seconds()
...
def subscription_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not current_user.has_library_access:
            flash("Your 7-day free trial has ended. Choose a subscription plan to continue using the library.", "warning")
...
CATEGORY_META = {
    "Codes": {"icon": "book-open", "short": "Codes", "description": "Standards & specifications"},
    "Materials": {"icon": "cube", "short": "Materials", "description": "Materials & product data"},
    "Quality Document Template": {"icon": "file-text", "short": "Templates", "description": "Document templates"},
    "Method Statement": {"icon": "clipboard", "short": "Methods", "description": "Method statements"},
    "Test Reports": {"icon": "flask", "short": "Reports", "description": "Test reports & certificates"},
    "Batch Plant": {"icon": "factory", "short": "Batch Plant", "description": "Plant approvals & mix designs"},
}
...
def get_categories() -> list[str]:
    ...
    return LIBRARY_CATEGORIES + [c for c in database_categories if c not in LIBRARY_CATEGORIES]
...
def get_category_counts() -> dict[str, int]:
    ...
    return {category: counts.get(category, 0) for category in get_categories()}

def resource_file_type(item: LinkItem) -> str:
    haystack = f"{item.url} {item.tags or ''}".lower()
    for ext in ("xlsx", "xls", "docx", "doc", "pdf", "pptx", "ppt"):
        if f".{ext}" in haystack or ext in (item.tags or "").lower().split(","):
            return ext.upper()
    return "LINK"
...
def ensure_database_schema() -> None:
    """Create or upgrade the database without deleting existing records."""
    db.create_all()

    backend = db.engine.url.get_backend_name()
    inspector = inspect(db.engine)

    # Keep compatibility with older deployments of the link library.
    if backend.startswith("postgresql"):
        link_statements = [
            ...
            "ALTER TABLE link_item ADD COLUMN IF NOT EXISTS material_type VARCHAR(150)",
            "ALTER TABLE link_item ADD COLUMN IF NOT EXISTS consumption_rate DOUBLE PRECISION",
            "ALTER TABLE link_item ADD COLUMN IF NOT EXISTS price_per_kg DOUBLE PRECISION",
    ...
    # Add material calculation columns safely to existing databases.
    inspector = inspect(db.engine)
    if "link_item" in inspector.get_table_names():
        link_columns = {column["name"] for column in inspector.get_columns("link_item")}
        add_prefix = "ADD COLUMN IF NOT EXISTS" if backend.startswith("postgresql") else "ADD COLUMN"
        if "material_type" not in link_columns:
            db.session.execute(text(f"ALTER TABLE link_item {add_prefix} material_type VARCHAR(150)"))
        if "consumption_rate" not in link_columns:
            float_type = "DOUBLE PRECISION" if backend.startswith("postgresql") else "FLOAT"
            db.session.execute(text(f"ALTER TABLE link_item {add_prefix} consumption_rate {float_type}"))
        if "price_per_kg" not in link_columns:
            float_type = "DOUBLE PRECISION" if backend.startswith("postgresql") else "FLOAT"
            db.session.execute(text(f"ALTER TABLE link_item {add_prefix} price_per_kg {float_type}"))
        db.session.commit()

    # db.create_all() does not add columns to an existing app_user table.
    # Add the one-time trial fields safely for both PostgreSQL and SQLite.
    if "app_user" in inspector.get_table_names():
        user_columns = {column["name"] for column in inspector.get_columns("app_user")}
        datetime_type = "TIMESTAMP" if backend.startswith("postgresql") else "DATETIME"
        add_column_prefix = "ADD COLUMN IF NOT EXISTS" if backend.startswith("postgresql") else "ADD COLUMN"
        if "trial_start" not in user_columns:
            db.session.execute(text(f"ALTER TABLE app_user {add_column_prefix} trial_start {datetime_type}"))
        if "trial_end" not in user_columns:
            db.session.execute(text(f"ALTER TABLE app_user {add_column_prefix} trial_end {datetime_type}"))
        db.session.execute(text("CREATE INDEX IF NOT EXISTS ix_app_user_trial_end ON app_user (trial_end)"))
        db.session.commit()

    # Give existing non-admin users without a recorded trial one trial only.
    # Active paid users are marked as having already passed the trial so they
    # cannot receive a new trial later after their subscription expires.
    now = utcnow()
    trial_delta = timedelta(days=free_trial_days())
    for user in User.query.filter(User.trial_end.is_(None)).all():
        user.trial_start = now
        if user.is_admin or user.has_active_subscription:
            user.trial_end = now
        else:
            user.trial_end = now + trial_delta
    db.session.commit()
...
def activate_subscription(user: User, plan: dict) -> None:
    now = utcnow()
    had_paid_subscription = user.has_active_subscription

    # Preserve any remaining trial time when the customer subscribes early.
    future_dates = [now]
    if user.subscription_end and user.subscription_end > now:
        future_dates.append(user.subscription_end)
    if user.trial_end and user.trial_end > now:
        future_dates.append(user.trial_end)
    base = max(future_dates)
...
def register_routes(app: Flask) -> None:
    ...
    def landing():
        if current_user.is_authenticated:
            if current_user.has_library_access:
    ...
    def register():
        ...
        if request.method == "POST":
            ...
            if len(name) < 2:
                ...
                flash(f"Your account has been created. Your {free_trial_days()}-day free trial is now active.", "success")
                return redirect(url_for("library"))
    ...
    def login():
        ...
        if request.method == "POST":
            ...
            if not user or not user.check_password(password):
                ...
                return redirect(url_for("library" if user.has_library_access else "subscribe"))
        ...
        if search_terms:
            ...
            for term in search_terms:
                ...
                search_filters.extend([
                    ...
                    LinkItem.material_type.ilike(like_text),
                ])
            query = query.filter(or_(*search_filters))

        browsing = bool(selected_category or search_terms)
        links = query.order_by(LinkItem.created_at.desc(), LinkItem.title.asc()).all() if browsing else []
        recent = LinkItem.query.order_by(LinkItem.created_at.desc()).limit(5).all()
        featured = LinkItem.query.order_by(LinkItem.created_at.desc()).limit(3).all()
        popular_codes = (
            LinkItem.query.filter(LinkItem.category == "Codes")
            .order_by(LinkItem.created_at.desc())
            .limit(5).all()
        )
        category_previews = {}
        for category in LIBRARY_CATEGORIES:
            category_previews[category] = (
                LinkItem.query.filter(LinkItem.category == category)
                .order_by(LinkItem.created_at.desc()).limit(3).all()
            )
    ...
    def save_link():
        ...
        def parse_non_negative_float(field_name: str):
            ...
            try:
                ...
                raise ValueError(f"{field_name.replace('_', ' ').title()} must be a number.")
            if value < 0:
                raise ValueError(f"{field_name.replace('_', ' ').title()} cannot be negative.")
            return value

        try:
            consumption_rate = parse_non_negative_float("consumption_rate")
            price_per_kg = parse_non_negative_float("price_per_kg")
        except ValueError as exc:
            flash(str(exc), "danger")
            return redirect(url_for("library", category=category or "Materials"))

        if category == "Materials" and (consumption_rate is None or price_per_kg is None):
            flash("Consumption rate and price per kg are required for materials.", "danger")
            return redirect(url_for("library", category="Materials"))
        ...
        try:
            if link_id:
                ...
                link.tags = tags
                link.notes = notes
                link.material_type = material_type if category == "Materials" else None
                link.consumption_rate = consumption_rate if category == "Materials" else None
                link.price_per_kg = price_per_kg if category == "Materials" else None
                ...
                db.session.add(
                    LinkItem(
                        ...
                        tags=tags,
                        notes=notes,
                        material_type=material_type if category == "Materials" else None,
                        consumption_rate=consumption_rate if category == "Materials" else None,
                        price_per_kg=price_per_kg if category == "Materials" else None,
                    )
                )
                flash("The resource has been added.", "success")
            db.session.commit()
    ...
    def admin_activate_user(user_id: int):
        ...
        activate_subscription(user, {"days": days})
        user.account_enabled = True
        db.session.commit()
        flash(f"{days} paid access day(s) were added for {user.name}.", "success")
