...
def subscription_required(view):
    ...
    def wrapped(*args, **kwargs):
        ...
        if not current_user.has_active_subscription:
            flash("يلزم وجود اشتراك ساري للدخول إلى مكتبة المحتوى.", "warning")
...
def create_or_promote_admin_from_env() -> None:
    ...
    name = (os.environ.get("ADMIN_NAME") or "مدير الموقع").strip()
...
def create_app(test_config: dict | None = None) -> Flask:
    ...
    login_manager.login_message = "سجّل الدخول أولًا للمتابعة."
    ...
    def enforce_enabled_account():
        if current_user.is_authenticated and not current_user.account_enabled:
            logout_user()
            flash("تم إيقاف هذا الحساب. تواصل مع إدارة الموقع.", "danger")
...
def register_routes(app: Flask) -> None:
    ...
    def register():
        ...
        if request.method == "POST":
            ...
            if len(name) < 2:
                flash("اكتب الاسم بشكل صحيح.", "danger")
            elif not re.fullmatch(r"[^@\s]+@[^@\s]+\.[^@\s]+", email):
                flash("اكتب بريدًا إلكترونيًا صحيحًا.", "danger")
            elif len(password) < 8:
                flash("كلمة المرور يجب ألا تقل عن 8 أحرف.", "danger")
            elif password != password_confirm:
                flash("كلمتا المرور غير متطابقتين.", "danger")
            elif User.query.filter(func.lower(User.email) == email).first():
                flash("هذا البريد مسجل بالفعل. سجّل الدخول بدلًا من إنشاء حساب جديد.", "warning")
            else:
                ...
                flash("تم إنشاء الحساب. اختر خطة الاشتراك لإتاحة المحتوى.", "success")
                return redirect(url_for("subscribe"))
    ...
    def login():
        ...
        if request.method == "POST":
            ...
            if not user or not user.check_password(password):
                ...
                flash("الحساب موقوف. تواصل مع إدارة الموقع.", "danger")
            else:
    ...
    def checkout(plan_code: str):
        ...
        if not publishable_key or not secret_key:
            flash("بوابة الدفع لم تُضبط بعد. أضف مفاتيح Moyasar في متغيرات البيئة.", "danger")
            return redirect(url_for("subscribe"))
    ...
    def payment_callback(token: str):
        if not current_user.is_authenticated:
            flash("سجّل الدخول بالحساب الذي بدأ عملية الدفع.", "warning")
            return redirect(url_for("login", next=request.full_path.rstrip("?")))
        ...
        if pending.expires_at < utcnow() and not pending.consumed_at:
            flash("انتهت صلاحية جلسة الدفع. ابدأ عملية جديدة.", "danger")
            return redirect(url_for("subscribe"))
        ...
        try:
            ...
            flash("لم يصل رقم عملية دفع صالح.", "danger")
            return redirect(url_for("subscribe"))
        ...
        if existing:
            if existing.user_id == current_user.id and existing.status == "paid":
                ...
                db.session.commit()
                flash("تم تسجيل هذه الدفعة مسبقًا، واشتراكك مفعل.", "success")
                return redirect(url_for("account"))
            flash("تعذر استخدام عملية الدفع هذه.", "danger")
            return redirect(url_for("subscribe"))
        ...
        try:
            ...
            flash("تعذر التحقق من بوابة الدفع الآن. أعد فتح رابط الرجوع بعد قليل.", "danger")
            return redirect(url_for("subscribe"))
        ...
        if not valid:
            ...
            flash("لم يتم اعتماد الدفعة لأن بياناتها لا تطابق خطة الاشتراك.", "danger")
            return redirect(url_for("subscribe"))
        ...
        try:
            ...
            payment = Payment(
                ...
                status="paid",
                provider_message=str(payment_data.get("message") or "")[:500],
                paid_at=utcnow(),
            )
            ...
            flash("تم الدفع لكن حدث خطأ أثناء تفعيل الاشتراك. تواصل مع الإدارة برقم العملية.", "danger")
            return redirect(url_for("subscribe"))

        flash("تم الدفع وتفعيل الاشتراك بنجاح.", "success")
        return redirect(url_for("library"))
    ...
    def save_link():
        ...
        if not title or not category or not url:
            flash("العنوان والتصنيف والرابط حقول مطلوبة.", "danger")
            return redirect(url_for("library"))
        if not url.lower().startswith(("http://", "https://")):
            flash("اكتب رابطًا صحيحًا يبدأ بـ https://", "danger")
            return redirect(url_for("library"))
        ...
        try:
            ...
            flash(f"تعذر حفظ الرابط: {exc}", "danger")
    ...
    def delete_link(link_id: int):
        ...
        try:
            ...
            db.session.rollback()
            flash(f"تعذر حذف الرابط: {exc}", "danger")
    ...
    def admin_activate_user(user_id: int):
        ...
        user.account_enabled = True
        db.session.commit()
        flash(f"تم تفعيل اشتراك {user.name} لمدة {days} يومًا.", "success")
    ...
    def admin_revoke_user(user_id: int):
        user = db.get_or_404(User, user_id)
        if user.is_admin:
            flash("لا يتم إلغاء صلاحية اشتراك حساب مدير.", "warning")
        else:
    ...
    def admin_toggle_user(user_id: int):
        ...
        if user.id == current_user.id:
            flash("لا يمكنك إيقاف حسابك الحالي.", "warning")
        else:
