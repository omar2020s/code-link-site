...
def ensure_database_schema():
    ...
    It does NOT delete saved links.
    """
    db.create_all()
...
def normalize_url(value: str) -> str:
    ...
    # If user writes something like google.com or www.google.com
...
def split_search_terms(search_text: str):
    """
    Allows searching using several keywords.
    Examples:
    - cement - datasheet
    - aramco, mot
    - repair / epoxy
...
def register_routes(app):
    ...
    def edit_link(link_id):
        edit_link = LinkItem.query.get_or_404(link_id)

        categories = get_categories()
        stats = get_dashboard_stats()
        selected_categories = [edit_link.category] if edit_link.category else []
